...
import { ClockIcon, FunnelIcon, ArrowPathIcon, MagnifyingGlassIcon } from '@heroicons/react/24/outline';
import marketingAllocationLogApi from '../../../api/marketingAllocationLogApi';
...
const OUTCOME_BADGE = {
  ...
  DUPLICATE: { bg: '#f3e8ff', fg: '#6b21a8', label: 'Duplicate' },
...
const MarketingAllocationHistory = () => {
  ...
  const [stats, setStats] = useState({ total: 0, assigned: 0, unassigned: 0, duplicate: 0, failed: 0 });
  ...
  const load = useCallback(async (silent = false) => {
    ...
    try {
      ...
      const [resp, statResp] = await Promise.all([
        marketingAllocationLogApi.getAll(params),
        marketingAllocationLogApi.getStats(params),
      ]);
      setRows(resp.data || []);
      setTotal(resp.meta?.total || 0);
      setStats(statResp.data || { total: 0, assigned: 0, unassigned: 0, duplicate: 0, failed: 0 });
      setLastUpdated(new Date());
    } catch (err) {
      if (!silent) toast.error(getErrorMessage(err, 'Failed to load allocation history'));
    } finally {
    ...
    <div style={{ animation: 'fadeIn 0.3s ease-out' }}>
      <div className="page-header flex-col md:flex-row md:items-center md:justify-between gap-3">
        <div className="page-header-left">
          <h1><ClockIcon style={{ width: 22, height: 22, marginRight: 6, verticalAlign: 'text-bottom' }} />Marketing Allocation History</h1>
          <p className="hidden sm:block">Every inbound marketing lead and the telecaller it was allocated to, with filters</p>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap' }}>
          ...
          >
            <span style={{
              ...
              animation: live ? 'mahPulse 1.5s infinite' : 'none',
            }} />
            {live ? 'Live' : 'Paused'}
          </button>
      ...
      <style>{`@keyframes mahPulse {0%{box-shadow:0 0 0 0 rgba(34,197,94,0.6)}70%{box-shadow:0 0 0 6px rgba(34,197,94,0)}100%{box-shadow:0 0 0 0 rgba(34,197,94,0)}}`}</style>
      ...
      <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap', marginBottom: 14 }}>
        ...
        <div className="crm-card" style={{ padding: '10px 16px' }}><div style={{ fontSize: 11, color: 'var(--text-muted)', textTransform: 'uppercase', fontWeight: 700 }}>Assigned</div><div style={{ fontSize: 20, fontWeight: 800, color: '#1e40af' }}>{stats.assigned}</div></div>
        ...
        <div className="crm-card" style={{ padding: '10px 16px' }}><div style={{ fontSize: 11, color: 'var(--text-muted)', textTransform: 'uppercase', fontWeight: 700 }}>Duplicate</div><div style={{ fontSize: 20, fontWeight: 800, color: '#6b21a8' }}>{stats.duplicate}</div></div>
      ...
      <div className="crm-card" style={{ padding: 14, marginBottom: 14, display: 'flex', gap: 12, flexWrap: 'wrap', alignItems: 'flex-end' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 4, flex: 1, minWidth: 180 }}>
          ...
          <div style={{ position: 'relative' }}>
            ...
            <input style={{ ...selectStyle, width: '100%', paddingLeft: 32 }} placeholder="Name / phone / campaign" value={filters.search} onChange={(e) => setFilter('search', e.target.value)} />
          </div>
        </div>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: 980 }}>
            ...
            <tbody>
              ...
              {!loading && rows.length === 0 && (
                <tr><td style={{ ...td, textAlign: 'center', color: 'var(--text-muted)' }} colSpan={7}>No allocation events match these filters</td></tr>
              )}
              {!loading && rows.map((r) => {
                ...
                return (
                  <tr key={r.id}>
                    ...
                    <td style={td}>{r.toUser ? userName(r.toUser) : '—'}</td>
...
export default MarketingAllocationHistory;
