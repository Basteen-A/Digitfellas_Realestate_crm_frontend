  ...
  const handleAccept = async (handoff) => {
    ...
    try {
      ...
      await leadWorkflowApi.acceptIncomingLead(handoff.leadId, {
        ...
        primaryRequirement: form.customerRequirement,
      ...
      try {
        ...
        if (quickWorkflowAction.needsSvDetails) {
          ...
          transPayload.primaryRequirement = form.customerRequirement;
                      ...
                      {roleActions.filter(a => a.tone === 'danger').map((action) => (
                        <button key={action.code} type="button"
                          ...
                          <div className="qa-drawer-st-icon">
                            {action.code.includes('JUNK') ? <NoSymbolIcon style={{ width: 18, height: 18 }} /> : action.code.includes('SPAM') ? <TrashIcon style={{ width: 18, height: 18 }} /> : <ExclamationTriangleIcon style={{ width: 18, height: 18 }} />}
                          </div>
                          <div className="qa-drawer-st-label">{action.label}</div>
                        </button>
                      ))}
